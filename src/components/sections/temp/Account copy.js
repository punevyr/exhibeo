...
const wallet = new BeaconWallet({ name: "Exhibeo" });
...
`;
  ...
  async function saveData() {
    ...
    const contract = await Tezos.wallet.at(EXHIBEO_CONTRACT);


    try {
      const result = await contract.methods.default(username,imga).send();
      console.log(result)
    } catch (error) {
      console.log(
